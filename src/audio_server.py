...
def resample_if_needed(y: np.ndarray, from_sr: int, to_sr: int) -> np.ndarray:
    if from_sr == to_sr:
        return y
    # librosa is not used here to avoid extra dependency in callback path; but we can afford it here
    import librosa
...
def stream_predict(
    ...
    input_sr = cfg.sample_rate  # we will request this from the device
    channels = 1
    ...
    print("Starting input stream... Press Ctrl+C to stop.")
    with sd.InputStream(
        device=device,
        channels=channels,
        samplerate=input_sr,
        ...
        while True:
            ...
            # block is already at cfg.sample_rate because we requested this samplerate from the stream
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Real-time microphone drone probability server")
