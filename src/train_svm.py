...
"""
This script trains a Support Vector Machine (SVM) model to classify audio files as containing drone or unknown sounds.

Usage:
    python src/train_svm.py --data-dir <data_directory> --model-path <model_output_path>

Parameters:
    --data-dir: The root directory containing training data classified into subdirectories 'drone' and 'unknown'.
    --model-path: Path to save the trained SVM model as a .joblib file.

Output:
    Trains an SVM model on the provided dataset and saves the trained model to the specified path.
