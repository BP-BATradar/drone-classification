# src/audio_server_gmm.py

"""
Real-time Audio Server for GMM Model

This script provides real-time audio classification using trained Gaussian Mixture Models (GMMs).
It captures audio from a microphone and continuously predicts whether the audio contains drone sounds.

Usage:
    python src/audio_server_gmm.py --model-path models/gmm_model.joblib --device 0 --mic-id "Main Mic"
    python src/audio_server_gmm.py --list-devices  # List available audio devices

Features:
    - Real-time audio capture from microphone
    - Live drone detection using GMM models
...
# Loads the GMM models, scaler, and configuration from the model bundle
...
# Real-time audio streaming and prediction using GMM models
def stream_predict(
    model_path: str,
    device: Optional[int],
    ...
    # Load the GMM models, scaler, and configuration
    ...
    with sd.InputStream(
        ...
        while True:
            ...
            # Compare log-likelihoods under each GMM
            logp_unknown = gmm_unknown.score_samples(feat_s)[0]
            logp_drone = gmm_drone.score_samples(feat_s)[0]
            
            # Convert log-likelihood ratio to probability (sigmoid-like mapping)
...
# Main function to set up and run the GMM audio server
def main() -> None:
