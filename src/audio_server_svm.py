# src/audio_server_svm.py

"""
Real-time Audio Server for SVM Model

This script provides real-time audio classification using a trained Support Vector Machine (SVM) model.
It captures audio from a microphone and continuously predicts whether the audio contains drone sounds.

Usage:
    python src/audio_server_svm.py --model-path models/svm_model.joblib --device 0 --mic-id "Main Mic"
    python src/audio_server_svm.py --list-devices  # List available audio devices

Features:
    - Real-time audio capture from microphone
    - Live drone detection using SVM model
...
# Loads the SVM model pipeline and configuration from the model bundle
...
# Resamples audio if needed using librosa
def resample_if_needed(y: np.ndarray, from_sr: int, to_sr: int) -> np.ndarray:
    if from_sr == to_sr:
        return y
    return librosa.resample(y.astype(np.float32), orig_sr=from_sr, target_sr=to_sr)

# Real-time audio streaming and prediction using SVM model
def stream_predict(
    model_path: str,
    device: Optional[int],
    ...
    # Load the SVM model pipeline and configuration
    ...
    print("Starting SVM input stream... Press Ctrl+C to stop.")
    ...
    with sd.InputStream(
        ...
        while True:
            ...
            # Ensure exact length, then extract features
            if len(block) < block_size:
                block = np.pad(block, (0, block_size - len(block)))
            else:
                block = block[:block_size]

            # Extract MFCC features (block is already at cfg.sample_rate because we requested this samplerate from the stream)
            ...
            # Predict drone probability using SVM pipeline
...
# Main function to set up and run the SVM audio server
def main() -> None:
    parser = argparse.ArgumentParser(description="Real-time microphone drone probability server (SVM)")
    parser.add_argument("--model-path", default="models/svm_model.joblib", type=str)
